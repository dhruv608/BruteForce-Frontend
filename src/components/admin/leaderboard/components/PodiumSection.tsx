...
export function PodiumSection({ top3, loading, error, mode = 'admin' }: any) {

  // Dynamic Rank logic mapping
const getRank = (entry: any) => {
  // The backend already returns the correct rank based on type
  // For podium, we'll use the rank provided by the backend
  return entry.global_rank || entry.city_rank || 0;
};

  const processedTop3 = useMemo(() => {
    if (!top3 || top3.length === 0) return [];

    // Add computed rank to each entry
    return top3.map((entry: any) => ({
      ...entry,
  ...
  return (
    <div className="relative pt-12 pb-8 bg-gradient-to-b from-primary/5 via-card to-card border border-border rounded-xl shadow-sm overflow-hidden flex items-end justify-center gap-4 md:gap-12 min-h-[350px]">
      ...
      {/* Map over podium positions (2nd, 1st, 3rd) for Left-Center-Right placing */}
      {[2, 1, 3].map((podiumPosition) => {
        // Get student by rank: 1st place gets index 0, 2nd place gets index 1, 3rd place gets index 2
        const student = processedTop3[podiumPosition - 1];
        ...
        const config = rankConfig[podiumPosition as keyof typeof rankConfig];
        ...
        return student ? (
          ...
          <div 
            ...
            style={{ animationDelay: `${podiumPosition * 0.2}s` }}
          >
            ...
            <div className={`w-full ${config.height} bg-card border-x border-t border-border rounded-t-2xl flex flex-col items-center justify-start pt-10 pb-4 relative overflow-hidden shadow-xl`}>
              <div className={`absolute inset-0 bg-gradient-to-t ${config.isCenter ? 'from-yellow-500/10' : podiumPosition === 2 ? 'from-slate-500/10' : 'from-amber-600/10'} to-transparent pointer-events-none`} />
